...
export default function LeadForm({ project }: { project?: string }) {
  ...
  useEffect(() => {
    if (!localStorage.getItem('altina_ft_ts')) {
      ...
      const utm = getParams(['utm_source', 'utm_medium', 'utm_campaign', 'utm_term', 'utm_content', 'gclid', 'fbclid', 'msclkid'])
  ...
  return (
    <form onSubmit={onSubmit} id="lead" className="grid gap-3">
      <label className="text-xs h-caps opacity-80">Name
        <input
          name="name"
          required
      ...
      <label className="text-xs h-caps opacity-80">Phone
        <input
          name="phone"
          required
          placeholder="Phone"
      ...
      <label className="text-xs h-caps opacity-80">Email (optional)
        <input
          name="email"
          type="email"
          placeholder="Email"
      ...
      {status === 'ok' && (
        <p className="text-green-400 text-xs">Thanks! We’ll reach out shortly.</p>
      )}
      {status === 'err' && (
        <p className="text-red-400 text-xs">Something went wrong. Please WhatsApp us.</p>
      )}
    </form>
  )
}