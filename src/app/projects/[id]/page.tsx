...
// --- Metadata (OpenGraph stays "website")
export async function generateMetadata(
  ...
  const title = `${p.name}${p.city ? ` in ${p.city}` : ""} | ${p.developer ? `${p.developer} • ` : ""}ALTINA™ Livings`;
  const description =
    [
      p.description,
      p.configuration,
  ...
  return {
    ...
    openGraph: {
      ...
      images: p.hero ? [{ url: abs(p.hero)!, width: 1200, height: 630, alt: p.name }] : undefined,
      type: "website",
    },
    twitter: {
